...
First, we will count the number of `numeric` columns and `character` columns in each of the four datasets. A `numeric` column represents that the observation is quantitative in nature and can be represented by a numerical value, whereas a `character` column has its values stored as text, and typically represents a qualitative or categorical variable. This will help us understand the values stored in our datasets
...
Next, for each of the four datasets, we will use the `glimpse` function from `dplyr` to look at some of the values stored in each column of the dataset. In addition to showing a snippet of the values in the dataset, this function also displays the column names and the data types for the variables in each of the columns, which lets us get a better feel for the data as a whole, and can help us evaluate how 'tidy' each of these datasets are.
```{r exploration-2}
...
Lastly, we will calculate the proportion of missing values in each of the four datasets. This step will be very helpful in choosing a dataset to analyze further, as it is often easier to work with a dataset with few or no missing entries. 
...
Based on the plot above, we see that there are only two variables with high proportions of unrecorded values, which are the `amenities` and `prop_management_company_name` variables. 

It is likely that a missing value in the `amenities` category is meant to represent that the apartment building does not offer any amenities, as opposed to representing that the data is missing for that building. Similarly, a missing value in the `prop_management_company_name` variable may mean that the building is not managed by a property management company, as opposed to implying that the property management company is simply unknown.
