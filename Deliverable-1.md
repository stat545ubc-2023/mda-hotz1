...
columns in each of the four datasets. A `numeric` column represents that
the observation is quantitative in nature and can be represented by a
numerical value, whereas a `character` column has its values stored as
text, and typically represents a qualitative or categorical variable.
This will help us understand the values stored in our datasets
...
cat(paste("The `apt_buildings` dataset has", apts_numeric_cols, "numeric variables and", 
          apts_character_cols, "text-based variables.\n"))
cat(paste("The `cancer_sample` dataset has", cancer_numeric_cols, "numeric variables and", 
          cancer_character_cols, "text-based variables.\n"))
cat(paste("The `parking_meters` dataset has", parking_numeric_cols, "numeric variables and", 
          parking_character_cols, "text-based variables.\n"))
cat(paste("The `steam_games` dataset has", steam_numeric_cols, "numeric variables and", 
          steam_character_cols, "text-based variables.\n"))
```

    The `apt_buildings` dataset has 9 numeric variables and 28 text-based variables.
    The `cancer_sample` dataset has 31 numeric variables and 1 text-based variables.
    The `parking_meters` dataset has 2 numeric variables and 20 text-based variables.
    The `steam_games` dataset has 4 numeric variables and 17 text-based variables.
...
dataset. In addition to showing a snippet of the values in the dataset,
this function also displays the column names and the data types for the
variables in each of the columns, which lets us get a better feel for
the data as a whole, and can help us evaluate how ‘tidy’ each of these
datasets are.
...
the four datasets. This step will be very helpful in choosing a dataset
...
`prop_management_company_name` variables.

It is likely that a missing value in the `amenities` category is meant
to represent that the apartment building does not offer any amenities,
as opposed to representing that the data is missing for that building.
